...
fun SettingPage(
    ...
    Scaffold(
        ...
        LazyColumn(contentPadding = PaddingValues(start = 16.dp, end = 16.dp, top = innerPadding.calculateTopPadding(), bottom = innerPadding.calculateBottomPadding()),
            verticalArrangement = Arrangement.spacedBy(8.dp)) {
            item{
                Spacer(modifier = Modifier.height(if(layoutType == SettingLayoutType.SPLIT) 24.dp else 16.dp))
            }
            item {
                SettingCategoryItem(title = "通用", icon = Icons.Outlined.Settings, selected = layoutType == SettingLayoutType.SPLIT && state.selected == Setting.GENERAL) {
            ...
            item {
                SettingCategoryItem(title = "扩展", icon = Icons.Outlined.LibraryAdd, selected = layoutType == SettingLayoutType.SPLIT && state.selected == Setting.ADDONS) {
            ...
            item {
                SettingCategoryItem(title = "界面", icon = Icons.Outlined.Palette, selected = layoutType == SettingLayoutType.SPLIT && state.selected == Setting.APPEARANCE) {
            ...
            item {
                SettingCategoryItem(title = "通知", icon = Icons.Outlined.EditNotifications, selected = layoutType == SettingLayoutType.SPLIT && state.selected == Setting.NOTIFICATION) {
            ...
            item {
                SettingCategoryItem(title = "存储空间", icon = Icons.Outlined.Storage, selected = layoutType == SettingLayoutType.SPLIT && state.selected == Setting.STORAGE) {
            ...
            item {
                SettingCategoryItem(title = "试验性", icon = Icons.Outlined.Science, selected = layoutType == SettingLayoutType.SPLIT && state.selected == Setting.EXPERIMENTAL) {
            ...
            item {
                SettingCategoryItem(title = "帮助与支持", icon = Icons.AutoMirrored.Outlined.HelpOutline, selected = layoutType == SettingLayoutType.SPLIT && state.selected == Setting.HELP) {
